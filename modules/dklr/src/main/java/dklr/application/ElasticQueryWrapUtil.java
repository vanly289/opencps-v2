...
public class ElasticQueryWrapUtil {
	...
	public static JSONObject queryES(String urlQuery, String method, String body) {
		_log.info("====START " + method + " ES==== ");
	...
	public static int DELETE(String key, String code, long conpanyId, String type_es) {
		...
		String body = "{\"query\":{\"bool\":{\"must\":[{\"match\":{\"type_es\":\"" + type_es + "\"}},{\"match\":{\""
				+ key + "\":\"" + code + "\"}}]}}}";
		...
		if (arrayToDelete.length() > 0) {
			if (arrayToDelete.getJSONObject("hits").getInt("total") > 0) {
				...
				for (int index = 0; index < arrayToDelete.getJSONObject("hits").getJSONArray("hits")
						.length(); index++) {
					try {
						String idRemove = arrayToDelete.getJSONObject("hits").getJSONArray("hits").getJSONObject(index)
								.getString("_id");
