...
public class ServiceProcessRolePersistenceImpl extends BasePersistenceImpl<ServiceProcessRole>
	...
	protected ServiceProcessRole removeImpl(
		...
		serviceProcessRole = toUnwrappedModel(serviceProcessRole);
	...
	public ServiceProcessRole updateImpl(ServiceProcessRole serviceProcessRole) {
		...
		if (isNew || !ServiceProcessRoleModelImpl.COLUMN_BITMASK_ENABLED) {
			finderCache.clearCache(FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION);
		}
	...
	protected ServiceProcessRole toUnwrappedModel(
		ServiceProcessRole serviceProcessRole) {
		if (serviceProcessRole instanceof ServiceProcessRoleImpl) {
			return serviceProcessRole;
		}

		ServiceProcessRoleImpl serviceProcessRoleImpl = new ServiceProcessRoleImpl();

		serviceProcessRoleImpl.setNew(serviceProcessRole.isNew());
		serviceProcessRoleImpl.setPrimaryKey(serviceProcessRole.getPrimaryKey());

		serviceProcessRoleImpl.setUuid(serviceProcessRole.getUuid());
		serviceProcessRoleImpl.setServiceProcessId(serviceProcessRole.getServiceProcessId());
		serviceProcessRoleImpl.setRoleId(serviceProcessRole.getRoleId());
		serviceProcessRoleImpl.setModerator(serviceProcessRole.isModerator());
		serviceProcessRoleImpl.setCondition(serviceProcessRole.getCondition());
