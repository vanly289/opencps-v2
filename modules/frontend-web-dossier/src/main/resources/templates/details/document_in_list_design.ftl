<div id="list_document_in_template" class="hidden">
	<v-layout wrap class="px-4 align-center-flex row-list-style"> 
		<v-flex xs12 class="mb-2">
			...
			<div style="margin-left: 40px;"><span @click="viewDossierFileVersionDialog(item, 'IN')" style="cursor: pointer;">{{item.partName}}</span> &nbsp; <span v-if="item.required">(<span style="color: red;">*</span>)</span></div>
		</v-flex>
	</v-layout>
</div>
