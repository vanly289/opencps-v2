...
(function($) {
	...
	function getJobPost() {
		return new Promise(function (resolve, reject) {
			$.ajax({
				url: "${api.endpoint}/employees/" + 0 + "/jobpos",
				dataType: "json",
				type: "GET",
				headers: {
					groupId: ${groupId},
	...
	function createJobPost (objTemp) {
		console.log('createjob');
		var url = "${api.endpoint}/employees/" + objTemp.employeeId + "/jobpos";
		$.ajax({
			url: url,
			headers: {
				"groupId": ${groupId}
			...
			data: {
				...
				workingUnitId: objTemp.workingUnitId,
				jobPosId: objTemp.jobPosId,
				mainJobPos: "true"
	...
	// function getWorkingUnitByEmployee (employeeId) {
	// 	return new Promise(function (resolve, reject) {
	// 		$.ajax({
	// 			url: "",
	// 			dataType: "json",
	...
	$(document).on('click', '#employee-create-submit-btn', function(event){
		...
		$.ajax({
			...
			success: function(data, textStatus, xhr) {
				...
				getJobPost().then(function (resultJob) {
					if (resultJob.data) {
						console.log('resultJob.data------------',resultJob.data)
						var jobPostTemp = resultJob.data[0];
						jobPostTemp['employeeId'] = data.employeeId;
						console.log('jobPostTemp------------', jobPostTemp)
						createJobPost(jobPostTemp);
					} else {
						console.log('no jobposts!');
					}
				}).catch(function (xhr) {
