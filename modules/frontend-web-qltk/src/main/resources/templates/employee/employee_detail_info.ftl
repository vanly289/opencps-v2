...
<div class="employee-deatil-info-wraper">
	...
	<div class="row MB10" id="divResetNewPass">
		<div class="col-xs-12 col-sm-8">
			<input type="text" class="form-control" id="resetNewPass" name="resetNewPass" placeholder="Nhập mật khẩu ban đầu">
		</div>
		<div class="col-xs-12 col-sm-4">
			<button class="btn btn-active border-rad-4" type="button" id="btnResetNewPass" name="btnResetNewPass">Tạo mới</button>
		</div>
	</div>
	<div class="row">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-active border-rad-4 MR10" data-pk="${(employee.employeeId)!}" onclick="changePassEmployee(this)">Đồng ý</button>
...
function showDialogChangePass () {
	// if () {
	// 	notification.show({
	// 		message: "Bạn phải mở tài khoản trước khi thực hiện hành động này!"
	// 	}, "error");
	// 	return;
	// }
...
function changePassEmployee (btn) {
	...
	if (validator.validate()) {
		var employeeId = $(btn).attr('data-pk');
		if (employeeId) {
			fnChangePass(employeeId);
		}
	}
...
function checkNewAcc () {
	...
	$.ajax({
		url: "${api.server}/employees/checkstatus/" + employeeId,
		dataType: "json",
		type: "GET",
		headers: {
			"groupId": ${groupId}
		...
		success: function(result){
			if (!result.isSetupComplete) {
				$('#divResetNewPass').show();
			} else {
				$('#divResetNewPass').hide();
			}
		},
		error: function(xhr){
			$('#divResetNewPass').hide();
...
function fnChangePass (employeeId) {
	$.ajax({
		url: "${api.server}/employees/changepass/" + employeeId,
		dataType: "json",
		type: "PUT",
		headers: {
			"groupId": ${groupId},
			password : $("#newEmployeePass").val()
		},
		data: {
		},
		success: function(result){
			...
			$('#changePassEmployeeDialog').modal('hide');
		},
		error: function(xhr){
			notification.show({
