...
public class VRProductClassificationFinderImpl extends VRProductClassificationFinderBaseImpl
		implements VRProductClassificationFinder {

	Log log = LogFactoryUtil.getLog(VRProductClassificationFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>VRProductClassificationFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<VRProductClassification> vrProductClassifications = (List<VRProductClassification>) QueryUtil.list(q, getDialect(),
						start, end);
				if (vrProductClassifications != null) {
					for (VRProductClassification vrProductClassification : vrProductClassifications) {
						JSONObject json = ActionUtil.object2Json(vrProductClassification,
								VRProductClassificationModelImpl.class, "");
						results.put(json);
					}
				}
			} else {
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VRProductClassificationFinder " + sql);
