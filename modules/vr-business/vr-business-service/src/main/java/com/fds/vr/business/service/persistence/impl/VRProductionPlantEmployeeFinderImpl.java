...
public class VRProductionPlantEmployeeFinderImpl extends VRProductionPlantEmployeeFinderBaseImpl
		implements VRProductionPlantEmployeeFinder {

	Log log = LogFactoryUtil.getLog(VRProductionPlantEmployeeFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>VRProductionPlantEmployeeFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<VRProductionPlantEmployee> vrProductionPlantEmployees = (List<VRProductionPlantEmployee>) QueryUtil.list(q, getDialect(),
						start, end);
				if (vrProductionPlantEmployees != null) {
					for (VRProductionPlantEmployee vrProductionPlantEmployee : vrProductionPlantEmployees) {
						JSONObject json = ActionUtil.object2Json(vrProductionPlantEmployee,
								VRProductionPlantEmployeeModelImpl.class, "");
						results.put(json);
					}
				}
			} else {
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VRProductionPlantEmployeeFinder " + sql);
