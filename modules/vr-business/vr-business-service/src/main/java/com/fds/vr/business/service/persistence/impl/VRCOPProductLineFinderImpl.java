...
public class VRCOPProductLineFinderImpl extends VRCOPProductLineFinderBaseImpl implements VRCOPProductLineFinder {

	Log log = LogFactoryUtil.getLog(VRCOPProductLineFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>VRCOPProductLineFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<VRCOPProductLine> vrcopProductLines = (List<VRCOPProductLine>) QueryUtil.list(q, getDialect(),
						start, end);
				if (vrcopProductLines != null) {
					for (VRCOPProductLine vrcopProductLine : vrcopProductLines) {
						JSONObject json = ActionUtil.object2Json(vrcopProductLine, VRCOPProductLineModelImpl.class,
								"");
						results.put(json);
					}
				}
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VRCOPProductLineFinder " + sql);
