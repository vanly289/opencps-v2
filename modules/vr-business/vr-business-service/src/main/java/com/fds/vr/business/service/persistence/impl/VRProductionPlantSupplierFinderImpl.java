...
public class VRProductionPlantSupplierFinderImpl extends VRProductionPlantSupplierFinderBaseImpl
		implements VRProductionPlantSupplierFinder {

	Log log = LogFactoryUtil.getLog(VRProductionPlantSupplierFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>VRProductionPlantSupplierFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<VRProductionPlantSupplier> vrProductionPlantSuppliers = (List<VRProductionPlantSupplier>) QueryUtil.list(q,
						getDialect(), start, end);
				if (vrProductionPlantSuppliers != null) {
					for (VRProductionPlantSupplier vrProductionPlantSupplier : vrProductionPlantSuppliers) {
						JSONObject json = ActionUtil.object2Json(vrProductionPlantSupplier,
								VRProductionPlantSupplierModelImpl.class, "");
						results.put(json);
					}
				}
			} else {
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VRProductionPlantSupplierFinder " + sql);
