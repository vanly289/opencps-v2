...
public class VRCorporationViewFinderImpl extends VRCorporationViewFinderBaseImpl
	implements VRCorporationViewFinder {

	Log log = LogFactoryUtil.getLog(VRCorporationViewFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>VRCorporationViewFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<VRCorporationView> vrCorporationViews = (List<VRCorporationView>) QueryUtil.list(q, getDialect(),
						start, end);
				if (vrCorporationViews != null) {
					for (VRCorporationView vrCorporationView : vrCorporationViews) {
						JSONObject json = ActionUtil.object2Json(vrCorporationView,
								VRCorporationViewModelImpl.class, "");
						results.put(json);
					}
				}
			} else {
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VRCorporationViewFinder " + sql);
