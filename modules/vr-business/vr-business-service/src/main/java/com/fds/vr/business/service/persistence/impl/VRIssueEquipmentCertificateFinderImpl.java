...
public class VRIssueEquipmentCertificateFinderImpl extends VRIssueEquipmentCertificateFinderBaseImpl
		implements VRIssueEquipmentCertificateFinder {

	Log log = LogFactoryUtil.getLog(VRIssueEquipmentCertificateFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>VRIssueEquipmentCertificateFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<VRIssueEquipmentCertificate> vrIssueEquipmentCertificates = (List<VRIssueEquipmentCertificate>) QueryUtil.list(q, getDialect(),
						start, end);
				if (vrIssueEquipmentCertificates != null) {
					for (VRIssueEquipmentCertificate vrIssueEquipmentCertificate : vrIssueEquipmentCertificates) {
						JSONObject json = ActionUtil.object2Json(vrIssueEquipmentCertificate,
								VRIssueEquipmentCertificateModelImpl.class, "");
						results.put(json);
					}
				}
			} else {
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VRIssueEquipmentCertificateFinder " + sql);
