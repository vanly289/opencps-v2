...
public class VRApplicantProfileHistoryFinderImpl extends VRApplicantProfileHistoryFinderBaseImpl
	implements VRApplicantProfileHistoryFinder {

	Log log = LogFactoryUtil.getLog(VRApplicantProfileHistoryFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes,
		...
		try {
			...
			log.info("===>>>VRApplicantProfileHistoryFinder " + sql + "|" + start + "|"
					+ end);
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VRApplicantProfileHistoryFinder " + sql);
