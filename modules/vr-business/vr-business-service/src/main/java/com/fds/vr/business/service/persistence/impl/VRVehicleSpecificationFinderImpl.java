...
public class VRVehicleSpecificationFinderImpl extends VRVehicleSpecificationFinderBaseImpl
		implements VRVehicleSpecificationFinder {

	Log log = LogFactoryUtil.getLog(VRVehicleSpecificationFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>>>>>>>>>>> VRVehicleSpecificationFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<VRVehicleSpecification> vrVehicleSpecifications = (List<VRVehicleSpecification>) QueryUtil.list(q,
						getDialect(), start, end);

				if (vrVehicleSpecifications != null) {
					for (VRVehicleSpecification vrVehicleSpecification : vrVehicleSpecifications) {
						JSONObject json = ActionUtil.object2Json(vrVehicleSpecification,
								VRVehicleSpecificationModelImpl.class, "");
						results.put(json);
					}
				}
			} else {
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VRVehicleSpecificationFinder " + sql);
