...
public class VROutputSheetFinderImpl extends VROutputSheetFinderBaseImpl implements VROutputSheetFinder {

	Log log = LogFactoryUtil.getLog(VROutputSheetFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>VROutputSheetFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<VROutputSheet> vrOutputSheets = (List<VROutputSheet>) QueryUtil.list(q, getDialect(),
						start, end);
				if (vrOutputSheets != null) {
					for (VROutputSheet vrOutputSheet : vrOutputSheets) {
						JSONObject json = ActionUtil.object2Json(vrOutputSheet, VROutputSheetModelImpl.class, "");
						results.put(json);
					}
				}
			} else {
	...
	public long countData(String sql) throws SystemException {
		...
		try {
			...
			log.info("===>>>VROutputSheetFinder " + sql);
