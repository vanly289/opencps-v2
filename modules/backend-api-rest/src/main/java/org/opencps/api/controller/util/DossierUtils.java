		...
		if (docs != null && docs.size() > 0) {
			for (Document doc : docs) {
				...
				if (Validator.isNotNull(doc.get(DossierTerm.SUBMIT_DATE))) {
					...
					Date submitDate = APIDateTimeUtils.convertStringToDate(doc.get(DossierTerm.SUBMIT_DATE),
							APIDateTimeUtils._LUCENE_PATTERN);
					// _log.info("SUBMIT_DATE_DATEEEEEE: "+submitDate);
					model.setSubmitDate(
							APIDateTimeUtils.convertDateToString(submitDate, APIDateTimeUtils._NORMAL_PARTTERN));
				...
				if (Validator.isNotNull(doc.get(DossierTerm.RECEIVE_DATE))) {
					Date receiveDate = APIDateTimeUtils.convertStringToDate(doc.get(DossierTerm.RECEIVE_DATE),
							APIDateTimeUtils._LUCENE_PATTERN);
					model.setReceiveDate(
							APIDateTimeUtils.convertDateToString(receiveDate, APIDateTimeUtils._NORMAL_PARTTERN));
				} else {
				...
				if (Validator.isNotNull(doc.get("ngay_ky_cc"))) {
					Date certDate = APIDateTimeUtils.convertStringToDate(doc.get("ngay_ky_cc"),
							APIDateTimeUtils._LUCENE_PATTERN);
				...
				// List<Deliverable> deliverableList =
				// action.getDeliverableByState(sb.toString(), "2");
				//
...
//		_log.info("ouputs: "+ouputs.size());
	...
	public static DossierDetailModel mappingForGetDetail(Dossier input, long userId) {
		...
		if (Validator.isNotNull(input)) {
			try {
				Document dossierDoc = DossierLocalServiceUtil.getDossierById(input.getDossierId(),
						input.getCompanyId());
			...
			model.setApplicantIdDate(APIDateTimeUtils.convertDateToString(input.getApplicantIdDate(),
					APIDateTimeUtils._NORMAL_PARTTERN));
			...
			model.setDueDate(
					APIDateTimeUtils.convertDateToString(input.getDueDate(), APIDateTimeUtils._NORMAL_PARTTERN));
			...
			model.setCorrectingDate(APIDateTimeUtils.convertDateToString(input.getCorrecttingDate(),
					APIDateTimeUtils._NORMAL_PARTTERN));
			...
			if (input.getDossierActionId() != 0) {
				DossierAction dossierAction = DossierActionLocalServiceUtil
						.fetchDossierAction(input.getDossierActionId());
				...
				DictCollection dictCollection = DictCollectionLocalServiceUtil
						.fetchByF_dictCollectionCode("DOSSIER_STATUS", input.getGroupId());
				...
				if (Validator.isNotNull(statusCode) || Validator.isNotNull(subStatusCode)) {
					...
					if (dictItem != null) {
						...
						if (Validator.isNotNull(specialStatus) && Boolean.parseBoolean(specialStatus)) {
							DossierActionUser dau = DossierActionUserLocalServiceUtil
									.getByDossierAndUser(input.getDossierActionId(), userId);
							if (dau != null) {
