...
//				_log.info("formData: "+formData);
	...
	@Override
	public Response getFileByTypeCode(HttpServletRequest request, HttpHeaders header, Company company,
			Locale locale, User user, ServiceContext serviceContext, String agencyNo, String typeCode,
			String keyword, String start, String end, String applicantIdNo, String deliverableState) {
		...
		try {
			...
			if (docList.size() > 0) {
				String deliverableCode = docList.get(0).get(DeliverableTerm.DELIVERABLE_CODE);
				
				_log.info("****DELIVERABLE_CODE__" + deliverableCode);
