...
import org.opencps.thirdparty.system.nsw.model.ResultOfExtending;
import org.opencps.thirdparty.system.nsw.model.ResultOfExtending.ItineraryExtending;
import org.opencps.thirdparty.system.nsw.model.ResultOfExtending.TransportPermitExtending;
import org.opencps.thirdparty.system.nsw.model.ResultOfExtending.TypeOfExtending;
			...
			for (String returnDossierFile : returnDossierFilesArr) {
				if (templateNo.equals(returnDossierFile)) {
					...
					if (formDataObj.has("RegistrationNumber")) {
						resultOfExtending.setRegistrationNumber(formDataObj.getString("RegistrationNumber"));						
					}
					if (formDataObj.has("ImportDate")) {
						String expiredDateStr = formDataObj.getString("ImportDate");
						Date expiredDate = DateTimeUtils.convertStringToDate(expiredDateStr);
						
						resultOfExtending.setImportDate(DateTimeUtils.convertDateToString(expiredDate, DateTimeUtils._NSW_DATE_TIME_FORMAT));						
					}
					else {
						resultOfExtending.setImportDate(DateTimeUtils.convertDateToString(new Date(), DateTimeUtils._NSW_DATE_TIME_FORMAT));						
					}
