...
public class DictGroupFinderImpl extends DictGroupFinderBaseImpl implements DictGroupFinder {
	
	private static Log log = LogFactoryUtil.getLog(DictGroupFinderImpl.class);
	...
	public JSONArray findData(String sql, List<String> columnNames, List<String> dataTypes, Class<?> modelClazz,
		...
		try {
			...
			log.info("===>>>>>>>>>>>>> DictGroupFinder " + sql + "|" + start + "|" + end);
			...
			if (Validator.isNotNull(modelClassName) && modelClazz != null) {
				...
				List<DictGroup> vrProductionPlants = (List<DictGroup>) QueryUtil.list(q, getDialect(), start, end);

				if (vrProductionPlants != null) {
					for (DictGroup vrProductionPlant : vrProductionPlants) {
						JSONObject json = ActionUtil.object2Json(vrProductionPlant, DictGroupModelImpl.class, "");
						results.put(json);
					}
				}
			} else {
