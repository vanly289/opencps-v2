...
public class SQLQueryInstance {
	...
	public SQLQueryInstance(String sqlStatementPattern, LinkedHashMap<String, String> columnMap,
			...
			String className, String tableAlias, String joinStatements,String countPrimaryKey) {
		...
		String countStatemanent = StringPool.BLANK;
		
		if(Validator.isNotNull(countPrimaryKey)) {
			
			countStatemanent = sqlStatemanent.replace("[$STATEMENT_COLUMN$]",
					Validator.isNotNull(tableAlias) ? "count(DISTINCT " + tableAlias + "."+countPrimaryKey+") AS total"
							: "count(*) AS total");
		}	else {
			 countStatemanent = sqlStatemanent.replace("[$STATEMENT_COLUMN$]",
